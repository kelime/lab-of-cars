/*******************************************************************************
* File Name: ADC_Side_Rangefinder_theACLK.c
* Version 1.70
*
*  Description:
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_Start
********************************************************************************
* Summary:
...
void ADC_Side_Rangefinder_theACLK_Start(void) 
{
    ...
    ADC_Side_Rangefinder_theACLK_CLKEN |= ADC_Side_Rangefinder_theACLK_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_StopBlock
********************************************************************************
* Summary:
...
{
    if (ADC_Side_Rangefinder_theACLK_CLKEN & ADC_Side_Rangefinder_theACLK_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = ADC_Side_Rangefinder_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_Side_Rangefinder_theACLK_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(ADC_Side_Rangefinder_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_Side_Rangefinder_theACLK_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_StandbyPower
********************************************************************************
* Summary:
...
void ADC_Side_Rangefinder_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        ADC_Side_Rangefinder_theACLK_CLKSTBY &= ~ADC_Side_Rangefinder_theACLK_CLKSTBY_MASK;
    }
    else
    {
        ADC_Side_Rangefinder_theACLK_CLKSTBY |= ADC_Side_Rangefinder_theACLK_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void ADC_Side_Rangefinder_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = ADC_Side_Rangefinder_theACLK_GetSourceRegister();
    uint16 oldDivider = ADC_Side_Rangefinder_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_Side_Rangefinder_theACLK_CLKEN & ADC_Side_Rangefinder_theACLK_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(ADC_Side_Rangefinder_theACLK_DIV_PTR, clkDivider);
                ADC_Side_Rangefinder_theACLK_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
        ...
        else
        {
            ...
            ADC_Side_Rangefinder_theACLK_CLKEN |= enabled;
        }
    }
}
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(ADC_Side_Rangefinder_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void ADC_Side_Rangefinder_theACLK_SetModeRegister(uint8 clkMode) 
{
    ADC_Side_Rangefinder_theACLK_MOD_SRC |= clkMode & ADC_Side_Rangefinder_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void ADC_Side_Rangefinder_theACLK_ClearModeRegister(uint8 clkMode) 
{
    ADC_Side_Rangefinder_theACLK_MOD_SRC &= ~clkMode | ~ADC_Side_Rangefinder_theACLK_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_GetModeRegister
********************************************************************************
* Summary:
...
{
    return ADC_Side_Rangefinder_theACLK_MOD_SRC & ADC_Side_Rangefinder_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = ADC_Side_Rangefinder_theACLK_GetDividerRegister();
    uint8 oldSrc = ADC_Side_Rangefinder_theACLK_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        ADC_Side_Rangefinder_theACLK_MOD_SRC |= CYCLK_SSS;
        ADC_Side_Rangefinder_theACLK_MOD_SRC =
            (ADC_Side_Rangefinder_theACLK_MOD_SRC & ~ADC_Side_Rangefinder_theACLK_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return ADC_Side_Rangefinder_theACLK_MOD_SRC & ADC_Side_Rangefinder_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    ADC_Side_Rangefinder_theACLK_PHASE = clkPhase & ADC_Side_Rangefinder_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_Side_Rangefinder_theACLK_GetPhase
********************************************************************************
* Summary:
...
{
    return ADC_Side_Rangefinder_theACLK_PHASE & ADC_Side_Rangefinder_theACLK_PHASE_MASK;
}
