...
{
    if (PWM_Clock_CLKEN & PWM_Clock_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void PWM_Clock_SetModeRegister(uint8 clkMode) 
{
    PWM_Clock_MOD_SRC |= clkMode & PWM_Clock_MODE_MASK;
}
...
*
*******************************************************************************/
void PWM_Clock_ClearModeRegister(uint8 clkMode) 
{
    PWM_Clock_MOD_SRC &= ~clkMode | ~PWM_Clock_MODE_MASK;
}
...
{
    return PWM_Clock_MOD_SRC & PWM_Clock_MODE_MASK;
}
...
void PWM_Clock_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        PWM_Clock_MOD_SRC |= CYCLK_SSS;
        PWM_Clock_MOD_SRC =
            (PWM_Clock_MOD_SRC & ~PWM_Clock_SRC_SEL_MSK) | clkSource;
    }
