...
uint8 Camera_Comp_initVar = 0u;


/* static Camera_Comp_backupStruct  Camera_Comp_backup; */
#if (CY_PSOC5A)
    static Camera_Comp_LOWPOWER_BACKUP_STRUCT  Camera_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    uint8 Camera_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Camera_Comp_Init
********************************************************************************
*
* Summary:
...
void Camera_Comp_Init(void) 
{
    ...
    Camera_Comp_SetSpeed(Camera_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    if ( Camera_Comp_DEFAULT_HYSTERESIS == 0u )
    {
        Camera_Comp_CR |= Camera_Comp_HYST_OFF;
    }
    else
    {
        Camera_Comp_CR &= ~Camera_Comp_HYST_OFF; 
    ...
    /* Power down override feature is not supported for PSoC5A. */
        ...
        if ( Camera_Comp_DEFAULT_PWRDWN_OVRD == 0u )
        {
            Camera_Comp_CR &= ~Camera_Comp_PWRDWN_OVRD;
        }
        else 
        {
            Camera_Comp_CR |= Camera_Comp_PWRDWN_OVRD;
    ...
    Camera_Comp_CR |= Camera_Comp_MX_AO;

    /* Set default sync */
    Camera_Comp_CLK &= ~Camera_Comp_SYNCCLK_MASK;
    if ( Camera_Comp_DEFAULT_BYPASS_SYNC == 0u )
    {
        Camera_Comp_CLK |= Camera_Comp_SYNC_CLK_EN;
    }
    else
    {
        Camera_Comp_CLK |= Camera_Comp_BYPASS_SYNC;
    }
}


/*******************************************************************************
* Function Name: Camera_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    Camera_Comp_PWRMGR |= Camera_Comp_ACT_PWR_EN;
    Camera_Comp_STBY_PWRMGR |= Camera_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Camera_Comp_restoreReg == 1u)
        {
            Camera_Comp_CR = Camera_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Camera_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Camera_Comp_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Camera_Comp_initVar: Is modified when this function is called for the 
...
void Camera_Comp_Start(void) 
{
    ...
    if ( Camera_Comp_initVar == 0u )
    {
        Camera_Comp_Init();
        
        Camera_Comp_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Camera_Comp_Stop
********************************************************************************
*
* Summary:
...
void Camera_Comp_Stop(void) 
{
    ...
    #if (CY_PSOC5A)
        /* Enable the variable */
        Camera_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        Camera_Comp_lowPowerBackup.compCRReg = Camera_Comp_CR;
        Camera_Comp_CR = Camera_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Camera_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void Camera_Comp_SetSpeed(uint8 speed) 
{
    ...
    Camera_Comp_CR = (Camera_Comp_CR & ~Camera_Comp_PWR_MODE_MASK) |
                           (speed & Camera_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Camera_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Camera_Comp_TR = Camera_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Camera_Comp_TR0 = Camera_Comp_HS_TRIM_TR0;
            Camera_Comp_TR1 = Camera_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Camera_Comp_TR = Camera_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Camera_Comp_TR0 = Camera_Comp_LS_TRIM_TR0;
            Camera_Comp_TR1 = Camera_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Camera_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Camera_Comp_WRK & Camera_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Camera_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
void Camera_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Camera_Comp_WRK & Camera_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Camera_Comp_TR |= Camera_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Camera_Comp_TR0 |= Camera_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Camera_Comp_TR |= Camera_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Camera_Comp_TR1 |= Camera_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Camera_Comp_WRK & Camera_Comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Camera_Comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Camera_Comp_ZeroCal(void) 
{
    ...
    tmpSW0 = Camera_Comp_SW0;
    tmpSW2 = Camera_Comp_SW2;
    tmpSW3 = Camera_Comp_SW3;

     /* Clear routing for inP, retain routing for inN */
    Camera_Comp_SW0 = 0x00u;
    Camera_Comp_SW2 = 0x00u;
    Camera_Comp_SW3 = tmpSW3 & ~Camera_Comp_CMP_SW3_INPCTL_MASK;

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Camera_Comp_CR;
    Camera_Comp_CR |= (Camera_Comp_CAL_ON | Camera_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Camera_Comp_TR = Camera_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Camera_Comp_TR0 = Camera_Comp_DEFAULT_CMP_TRIM;
        Camera_Comp_TR1 = Camera_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Camera_Comp_CR & Camera_Comp_PWR_MODE_MASK) == Camera_Comp_PWR_MODE_FAST)
    {
        Camera_Comp_trimAdjust(0);
    }
    else /* default to trim for fast modes */
    {
        Camera_Comp_trimAdjust(1);
		Camera_Comp_trimAdjust(0);
    }
   
    /* Restore Config Register */
    Camera_Comp_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Camera_Comp_SW0 = tmpSW0;
    Camera_Comp_SW2 = tmpSW2;
    Camera_Comp_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Camera_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return (((uint16)Camera_Comp_TR1 << 8) | (Camera_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Camera_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void Camera_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Camera_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Camera_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
