...
void Clock_Counter_Start(void) 
{
    ...
    Clock_Counter_CLKEN |= Clock_Counter_CLKEN_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_Counter_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_Counter_CLKEN & Clock_Counter_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Clock_Counter_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Counter_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_Counter_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Counter_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_Counter_StandbyPower
********************************************************************************
* Summary:
...
void Clock_Counter_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_Counter_CLKSTBY &= ~Clock_Counter_CLKSTBY_MASK;
    }
    else
    {
        Clock_Counter_CLKSTBY |= Clock_Counter_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_Counter_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Counter_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = Clock_Counter_GetSourceRegister();
    uint16 oldDivider = Clock_Counter_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Counter_CLKEN & Clock_Counter_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_Counter_DIV_PTR, clkDivider);
                Clock_Counter_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
{
    return CY_GET_REG16(Clock_Counter_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Counter_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Counter_SetModeRegister(uint8 clkMode) 
{
    Clock_Counter_MOD_SRC |= clkMode & Clock_Counter_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Counter_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Counter_ClearModeRegister(uint8 clkMode) 
{
    Clock_Counter_MOD_SRC &= ~clkMode | ~Clock_Counter_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_Counter_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_Counter_MOD_SRC & Clock_Counter_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Counter_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_Counter_GetDividerRegister();
    uint8 oldSrc = Clock_Counter_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_Counter_MOD_SRC |= CYCLK_SSS;
        Clock_Counter_MOD_SRC =
            (Clock_Counter_MOD_SRC & ~Clock_Counter_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Counter_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_Counter_MOD_SRC & Clock_Counter_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Counter_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Clock_Counter_PHASE = clkPhase & Clock_Counter_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Counter_GetPhase
********************************************************************************
* Summary:
...
{
    return Clock_Counter_PHASE & Clock_Counter_PHASE_MASK;
}
