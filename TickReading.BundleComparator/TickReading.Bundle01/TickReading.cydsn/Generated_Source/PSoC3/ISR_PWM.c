...
void ISR_PWM_Start(void)
{
    ...
    /* Set the priority. */
    ISR_PWM_SetPriority(ISR_PWM_INTC_PRIOR_NUMBER);
...
CY_ISR(ISR_PWM_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (ISR_PWM__ES2_PATCH ))      
            ISR_PWM_ISR_PATCH();
        #endif
    #endif
}
...
*   the component has been started use ISR_PWM_StartEx instead.
*
*
* Parameters:
*   address: Address of the ISR to set in the interrupt vector table.
...
{
    CY_SET_REG16(ISR_PWM_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(ISR_PWM_INTC_VECTOR);
}
...
*   or ISR_PWM_StartEx will override any effect this method would have had. 
*	This method should only be called after ISR_PWM_Start or 
...
uint8 ISR_PWM_GetState(void) 
{
    ...
    return (*ISR_PWM_INTC_SET_EN & ISR_PWM__INTC_MASK) ? 1:0;
}
